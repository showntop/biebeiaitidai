...
```bash
npm install
npm test            # 全部规则单测与验收测试（当前 180+ 项，排除 sim）
npm run test:watch  # 监听
npm run typecheck   # 全量类型检查
npm run lint        # 架构边界与 JSON 配置静态检查
npm run sim         # 贪婪 bot 千局模拟，输出胜负/星级/峰值分布（平衡诊断）
npm run quality     # 提交前完整质量门禁
```

## 当前状态

**核心规则层已全部实现，180+ 项单测 + 类型检查全绿：**

| 系统 | 职责（对应策划文档） |
|---|---|
| types / EventBus / rng / config | 领域类型+事件契约、类型安全事件总线、可注入确定性 RNG、数据驱动配置与启动期 schema 校验 |
...
| PropSystem | 4 类纸团 CD/能量槛、轻点快投与按住拖甩、目标身份锁定、真实 Perfect 判定与可变奖励事件、Boss 资源保底、无损取消 |
...
**当前体验收口：**
1. 三类投掷道具统一为“一步操作”：轻点自动快投推荐目标；按住拖甩可精准换目标；拍马屁点按即放。
2. Perfect 奖励、阶段加速、命中/Miss、认可度、AI 状态和结算战报均有明确事件反馈。
3. 第一关覆盖轻点快投、按住拖甩、目标收益预告与金色 Perfect 准星教学，完成后自动消失。
