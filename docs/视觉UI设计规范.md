...
| **道具按钮** | 圆角方 + 居中符号 + 状态行(CD秒/充能%/×次数) | 160×80，圆角 12 | 4 个底部；未解锁灰、用尽红、就绪白 |
...
> 竖屏 1080×1920。**结构**：传送带(=AI显示器·任务流)在**上方**，AI角色在传送带**下方居中**面朝它（背对玩家），认可度条再往下，事件日志压薄放认可度条下方，**道具按钮下沉到最底部**（60s 内高频点按的核心交互区，占最靠拇指的黄金位置，比事件日志优先拿"最底"）。顶部HUD只放 设置/标题/计时器。
...
├──────────────────────────────┤
│[加需求剩5][改需求剩4][丢锅剩2][拍马屁]│ 道具按钮横排4个 (72~84%，宽矮比例贴底)
│                              │ 底部安全区 (84~100%，home indicator + 拇指留白)
└──────────────────────────────┘ 1920
```

- **分区**（自上而下）：顶部HUD → 传送带(任务流) → AI角色 → 认可度条 → 事件日志(薄) → 道具按钮(贴底) → 安全区。
...
- **道具按钮贴底、宽矮比例**：占屏高 ~12%（v3 的 16% 已减回 12%，避免按钮过高变柱状），按 §3 的 160×80(2:1) 比例横排，显示**剩N次**（对齐代码 `PropSystem.uses`），作为 60 秒内最高频的点按区。
- 角色分层 sprite（见 §4.1）；bg-office 仅环境。
- 主界面不靠 AI 出整图，由 Cocos 按此线框拼（`GameRunner` + `Game.scene`）。

> **已定**（与代码一致）：标题 = 「别让AI替代你」；道具按钮 = **4 个横排**（加需求 / 改需求 / 丢锅 / 拍马屁，拍马屁第 4 格）。
...
底部(72~84%，贴底，宽矮比例)：
- 横排 4 个圆角方形【道具按钮】：蓝"加需求 剩5"、紫"改需求 剩4"、红"丢锅 剩2"、粉"拍马屁"；按钮呈**宽矮比例**（宽>高，约2:1），**不要**画成高瘦的柱状；这一排位于全图**最下方**，但高度适中（约占屏高12%），下方留出底部安全区留白
...
BOTTOM (72-84%, near the screen edge, wide-and-short proportion): a row of 4 round-cornered SKILL
BUTTONS with Chinese labels: blue "加需求 剩5", purple "改需求 剩4", red "丢锅 剩2", pink "拍马屁".
